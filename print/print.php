 ...
 .qr-code {
    ...
    <div class="col-1"><img src="../img/Ministry_of_Health_and_Population_of_Egypt.png" class="img-fluid"
      ...
      <div class="dropdown show d-inline h3 col-4">
        ...
        $governorate = $_SESSION['governorate'];
        $nationality = $_POST['nationality'];
        $NID = $_POST['nationalId'];
        $FID = $_POST['FnationalId']; 
          if($nationality == 'مصرى'){
       $user = "SELECT * FROM users WHERE nationalId = '$NID' And nationalId != '' and governorate = '$governorate' ORDER BY date DESC limit 1";
...
<?php
       ...
       $user = "SELECT * FROM users WHERE FnationalId = '$FID' and FnationalId !='' and governorate = '$governorate' ORDER BY date DESC limit 1";
       $query = mysqli_query( $conn,$user) or die('error:'.mysqli_error($conn));
       $result = mysqli_fetch_array($query);
         $count= mysqli_num_rows($query);
          if($count > 0){        
